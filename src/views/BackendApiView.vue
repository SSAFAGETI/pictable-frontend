...
const apiGroups: ApiGroup[] = [
  ...
  },
  {
    ...
    endpoints: [
      ...
      { method: 'POST', path: '/comments/{commentId}/replies/', title: '작성자 답글', description: '레시피 작성자만 댓글에 공식 답글을 남길 수 있습니다.', request: { content: '참치 기름을 조금 빼면 더 담백해요.' }, success: { ok: true, data: { id: 'reply_2', commentId: 'comment_1', isAuthorReply: true } }, failure: { ok: false, error: { code: 'FORBIDDEN', message: '작성자만 답글을 남길 수 있습니다.' } } },
    ],
  },
  {
    id: 'notifications',
    ...
    endpoints: [
      ...
      { method: 'PATCH', path: '/notifications/read/', title: '알림 읽음 처리', description: '읽음 처리 후 빨간 카운트 배지를 즉시 갱신합니다.', request: { notificationIds: ['noti_1', 'noti_2'] }, success: { ok: true, data: { unreadCount: 0, updatedIds: ['noti_1', 'noti_2'] } }, failure: { ok: false, error: { code: 'NOTIFICATION_NOT_FOUND', message: '읽음 처리할 알림을 찾을 수 없습니다.' } } },
      ...
      { method: 'GET', path: '/users/me/summary/', title: '마이페이지 요약', description: '내 레시피, 저장, 좋아요, 구독/팔로워 카운트를 한 번에 내려줍니다.', request: null, success: { ok: true, data: { recipeCount: 8, savedCount: 12, likedCount: 21, followerCount: 5, followingCount: 4 } }, failure: { ok: false, error: { code: 'UNAUTHORIZED', message: '마이페이지는 로그인 후 사용할 수 있습니다.' } } },
    ],
  },
  {
    id: 'ingredients',
    ...
    icon: Utensils,
    endpoints: [
      { method: 'GET', path: '/ingredients/search/?q=계란', title: '재료 검색', description: '프론트는 20자 제한을 두고, 서버도 동일하게 검증합니다.', request: null, success: { ok: true, data: { items: [{ id: 'ingredient_1', name: '계란', aliases: ['달걀'] }], maxQueryLength: 20 } }, failure: { ok: false, error: { code: 'QUERY_TOO_LONG', message: '재료 검색어는 20자 이하로 입력해주세요.' } } },
      { method: 'POST', path: '/ingredients/detect/', title: 'AI 재료 인식', description: '냉장고 사진에서 재료 후보를 추출해 추천 검색으로 넘깁니다.', request: { imageId: 'media_fridge_1' }, success: { ok: true, data: { ingredients: [{ name: '계란', confidence: 0.94 }, { name: '김치', confidence: 0.88 }] } }, failure: { ok: false, error: { code: 'DETECTION_FAILED', message: '재료를 인식하지 못했습니다. 다시 촬영해주세요.' } } },
      ...
      { method: 'POST', path: '/media/upload/', title: '이미지 업로드', description: '레시피 대표 이미지, 단계 이미지, 재료 인식 이미지에 공통으로 사용합니다.', request: { file: 'multipart/form-data', purpose: 'recipe-step' }, success: { ok: true, data: { id: 'media_1', url: 'https://cdn.example.com/media_1.jpg', width: 1200, height: 900 } }, failure: { ok: false, error: { code: 'FILE_TOO_LARGE', message: '이미지는 10MB 이하만 업로드할 수 있습니다.' } } },
    ],
  },
  {
    id: 'cook-mode',
